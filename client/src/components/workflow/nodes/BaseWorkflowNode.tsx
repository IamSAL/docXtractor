  ...
  ({
    ...
    const getParamSummary = () => {
      ...
      if (params.extractorId)
        displayParams.push({ label: "Extractor", value: "Configured" });
      if (params.to) displayParams.push({ label: "To", value: params.to });
      if (params.webhookPath)
        displayParams.push({ label: "Path", value: params.webhookPath });
      if (params.cronExpression)
        displayParams.push({ label: "Cron", value: params.cronExpression });
      if (params.url) displayParams.push({ label: "URL", value: params.url });
      if (params.condition)
        displayParams.push({ label: "Condition", value: params.condition });
    ...
    return (
      ...
      >
        ...
        <div className="border-t-2 border-black bg-gray-50 px-3 py-1.5 flex items-center justify-between">
          ...
          {status === "running" && (
            <span className="text-[9px] font-bold text-yellow-600 uppercase animate-pulse">
              ● Processing
            </span>
          )}
          {status === "failed" && (
