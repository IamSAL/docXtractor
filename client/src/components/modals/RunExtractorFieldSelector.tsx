...
export function RunExtractorFieldSelector({
	...
	// Pre-select default variant on mount
	useEffect(() => {
		if (!variantId && variants?.length) {
		...
		<div className="flex flex-col gap-3">
			...
			{variants && variants.length > 0 && (
				<Select
					...
					onValueChange={(val) =>
						setValue("variantId", val === "__base__" ? undefined : val)
					}
				>
					<Select.Trigger className="border-2 border-black bg-white text-sm font-bold h-9">
						<Select.Value placeholder="Full Schema (All Fields)" />
					</Select.Trigger>
					<Select.Content>
						<Select.Group>
