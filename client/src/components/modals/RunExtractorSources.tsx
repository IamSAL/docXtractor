...
import type { RunExtractorFormData, ExtractionSource } from '@/types/run-extractor'
...
export function RunExtractorSources({ onUploadFile }: RunExtractorSourcesProps) {
    const { control, getValues, formState: { errors } } = useFormContext<RunExtractorFormData>()
    const { fields: sources, append, remove, update } = useFieldArray({
        control,
        name: 'sources'
    })
    ...
    const addFileSource = async (file: File) => {
        ...
        try {
            ...
            if (index !== -1) {
                update(index, { ...newSource, content: result.url, status: 'ready' })
            }
        } catch (error) {
            const currentSources = getValues('sources')
            const index = currentSources.findIndex(s => s.id === id)
            if (index !== -1) {
                update(index, { ...newSource, status: 'error', description: 'Upload failed' })
            }
        }
    }
        ...
        <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-2">
                <label className="text-xs font-bold uppercase tracking-wider text-gray-500 ml-1 flex justify-between items-center">
                    ...
                    <span className="text-[10px] bg-black text-white px-2 py-0.5 rounded-full">Supported: PDF, URL, DOCX</span>
                </label>
                <div className="relative group cursor-pointer">
                    ...
                    <div className="flex flex-row items-center gap-6 rounded-lg border-2 border-dashed border-gray-400 bg-gray-50 px-6 py-8 transition-all group-hover:border-black group-hover:bg-primary/10">
                        <div className="w-14 h-14 rounded-full bg-white border-2 border-black flex items-center justify-center shadow-hard-sm shrink-0">
                            <span className="material-symbols-outlined text-2xl text-black">add_to_photos</span>
                        </div>
                        <div className="flex flex-col">
                            <p className="text-black text-base font-bold">Drop files here to upload</p>
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {sources.map((source, index) => (
                            <div key={source.id} className="flex flex-col border-2 border-black bg-white shadow-hard-sm transition-all overflow-hidden group">
                                <div className="flex items-center justify-between p-3 border-b-2 border-black last:border-b-0">
                                    <div className="flex items-center gap-3 overflow-hidden flex-1">
                                        <div className={`w-8 h-8 border border-black flex items-center justify-center shrink-0 ${source.type === 'file' ? 'bg-red-100' : 'bg-blue-100'
                                            }`}>
                                            <span className={`material-symbols-outlined text-lg ${source.type === 'file' ? 'text-red-600' : 'text-blue-600'
                                                }`}>
                                        ...
                                        <div className="flex flex-col min-w-0 flex-1">
                                            ...
                                            <div className="flex items-center gap-2 text-[10px] text-gray-500 uppercase font-bold">
                                                {source.sizeBytes && <span>{(source.sizeBytes / 1024 / 1024).toFixed(1)} MB</span>}
                                                {source.sizeBytes && <span className="w-1 h-1 bg-gray-400 rounded-full"></span>}
                                                <span className={`${source.status === 'ready' ? 'text-green-600' :
                                                    source.status === 'uploading' ? 'text-blue-600 animate-pulse' : 'text-red-600'
                                                    } flex items-center gap-1`}>
                                                    <span className="material-symbols-outlined text-[10px]">
                                                        {source.status === 'ready' ? 'check_circle' :
                                                            source.status === 'uploading' ? 'sync' : 'error'}
                                                    </span>
                                ...
                                {source.status === 'ready' && source.content && (
                                    <div className="p-3 bg-gray-50 border-t-2 border-black flex justify-center items-center">
                                        {source.content.match(/\.(jpeg|jpg|gif|png|webp|svg|bmp|tiff|avif)/i) || source.content.startsWith('data:image') ? (
                                            <img
                                                src={source.content}
                                                alt={source.name}
                                            ...
                                            <div className="flex items-center gap-3 py-2">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <a href={source.content} target="_blank" rel="noopener noreferrer" className="text-[10px] text-primary hover:underline break-all">
                                                        View Full Document
                                                    </a>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-3 py-2">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-xs font-bold text-gray-700">Resource</span>
                                                    <a href={source.content} target="_blank" rel="noopener noreferrer" className="text-[10px] text-primary hover:underline break-all line-clamp-1">
                                                        {source.content}
                                                    </a>
                                                </div>
                                            </div>
