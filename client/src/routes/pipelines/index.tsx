            ...
            <div className="flex flex-col h-full p-4 lg:p-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 pb-12">
                    ...
                    {[
                        ...
                        <Card key={i} className="flex flex-col bg-white border-2 border-black rounded-sm overflow-hidden h-full hover:shadow-[6px_6px_0px_0px_#000000] transition-all duration-200 p-0" shadowsize="sm">
                            ...
                            <div className={`h-32 border-b-2 border-black relative overflow-hidden ${pipeline.status === 'Draft' ? 'bg-yellow-50' : pipeline.status === 'Config Error' ? 'bg-red-50' : 'bg-gray-100'}`}>
                                ...
                                <div className={`absolute top-3 right-3 text-black text-[10px] font-bold px-2 py-1 border border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] uppercase tracking-wider ${pipeline.status === 'Active' ? 'bg-green-400' : 'bg-white'}`}>
                                    {pipeline.status}
                                </div>
                            ...
                            <div className="p-5 flex-1 flex flex-col gap-4">
                                <div>
                                    <h3 className="text-xl font-bold leading-tight mb-1">{pipeline.title}</h3>
                                    <p className="text-xs font-mono text-gray-500">ID: {pipeline.id}</p>
                                </div>
                                ...
                                <div className="mt-auto pt-4 border-t-2 border-dashed border-gray-300 flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-gray-600">
                                        ...
                                        <span className="text-xs font-bold font-mono uppercase">{pipeline.fields}</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-gray-600">
                                        {pipeline.time && (
                                            <>
                                                <span className="material-symbols-outlined text-lg">timer</span>
                                                <span className="text-xs font-bold font-mono uppercase">{pipeline.time}</span>
                                            </>
                                        )}
                                        {pipeline.model && (
                                            <>
                                                <span className="material-symbols-outlined text-lg">psychology</span>
                                                <span className="text-xs font-bold font-mono uppercase">{pipeline.model}</span>
                                            </>
                                        )}
                                        {pipeline.error && (
                                            <div className="flex items-center gap-2 text-red-500 font-bold">
                                                <span className="material-symbols-outlined text-lg">error</span>
                                                <span className="text-xs font-mono uppercase">{pipeline.error}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-[1fr_1fr_auto] border-t-2 border-black divide-x-2 divide-black">
                                ...
                                <button className={`py-3 font-bold text-xs uppercase tracking-wider transition-colors flex items-center justify-center gap-2 group ${pipeline.status === 'Draft' ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-[#e5d161]/20 hover:bg-[#e5d161] text-black'}`}>
                                    <span className={`material-symbols-outlined text-sm ${pipeline.status === 'Draft' ? '' : 'group-hover:translate-x-1 transition-transform'}`}>
                                        {pipeline.status === 'Draft' ? 'block' : 'play_arrow'}
                                    </span>
                                    Run
                                </button>
                                <button className="px-4 hover:bg-red-500 hover:text-white transition-colors flex items-center justify-center" title="Delete Pipeline">
                    ...
                    >
                        ...
                        <h3 className="text-xl font-bold text-gray-500 group-hover:text-black mb-1 uppercase">Create New Pipeline</h3>
