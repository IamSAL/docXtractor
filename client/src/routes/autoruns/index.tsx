					...
					{isLoading ? (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
							{workflows?.map((workflow: any) => {
								...
								return (
									<div key={workflow.id} className={`bg-white border-3 border-black shadow-[4px_4px_0px_#000000] rounded-sm flex flex-col group h-full transition-all hover:bg-yellow-50/50 ${isPaused ? 'opacity-90' : ''}`}>
										<div className="p-5 border-b-3 border-black bg-gray-50 flex justify-between items-start">
											<div className="flex flex-col gap-1">
												<div className="flex items-center gap-2">
													<span className="size-3 rounded-full border border-black shadow-[1px_1px_0px_#000]" style={{ backgroundColor: getStatusColor(workflow.status) }}></span>
													<span className="text-xs font-bold uppercase tracking-wider">{workflow.status}</span>
												</div>
												<h3 className="text-xl font-bold leading-tight mt-1">{workflow.name}</h3>
											</div>
											<button
										...
										<div className={`p-6 flex items-center justify-between gap-2 bg-white relative overflow-hidden ${isPaused ? 'grayscale' : ''}`}>
											...
											<div className="relative z-10 flex flex-col items-center gap-1 group/node">
												<div className="size-10 bg-white border-2 border-black flex items-center justify-center shadow-[2px_2px_0px_#000] group-hover/node:-translate-y-1 transition-transform">
													<span className="material-symbols-outlined text-sm">{icons.icon1}</span>
												</div>
												<span className="text-[10px] font-bold uppercase">{icons.label1}</span>
											</div>
											<div className={`h-[3px] bg-black flex-grow relative after:content-[''] after:absolute after:right-[-2px] after:top-[-5px] after:border-t-[6px] after:border-t-transparent after:border-b-[6px] after:border-b-transparent after:border-left-[10px] after:border-left-black ${isPaused ? 'border-dashed' : ''}`}></div>
											...
											<div className="relative z-10 flex flex-col items-center gap-1 group/node">
												<div className="size-10 bg-[#ffdd00] border-2 border-black flex items-center justify-center shadow-[2px_2px_0px_#000] group-hover/node:-translate-y-1 transition-transform">
													<span className="material-symbols-outlined text-sm">{icons.icon2}</span>
												</div>
												<span className="text-[10px] font-bold uppercase">{icons.label2}</span>
											</div>
											<div className={`h-[3px] bg-black flex-grow relative after:content-[''] after:absolute after:right-[-2px] after:top-[-5px] after:border-t-[6px] after:border-t-transparent after:border-b-[6px] after:border-b-transparent after:border-left-[10px] after:border-left-black ${isPaused ? 'border-dashed' : ''}`}></div>

											<div className="relative z-10 flex flex-col items-center gap-1 group/node">
												<div className="size-10 bg-white border-2 border-black flex items-center justify-center shadow-[2px_2px_0px_#000] group-hover/node:-translate-y-1 transition-transform">
													<span className="material-symbols-outlined text-sm">{icons.icon3}</span>
												</div>
												<span className="text-[10px] font-bold uppercase">{icons.label3}</span>
											</div>
										</div>
										<div className="px-5 py-3 border-t-3 border-black bg-[#F7F7F5] flex justify-between items-center text-sm font-medium">
											<div className="flex items-center gap-2">
												...
												<span>{workflow.lastTriggeredAt ? formatDistanceToNow(new Date(workflow.lastTriggeredAt), { addSuffix: true }) : 'Never'}</span>
											</div>
											<div className="flex items-center gap-2">
												<span className="font-bold uppercase text-xs">Nodes:</span>
												<span className="bg-black text-[#33FF57] px-1.5 font-mono">{workflow.definition?.nodes?.length || 0}</span>
											</div>
										</div>
							...
							>
								...
								<h3 className="text-xl font-bold uppercase text-black">Create New Workflow</h3>
								<p className="text-sm font-medium mt-2 text-gray-600">Design your automation workflow</p>
							</Link>
						</div>
					)}
				</div>
