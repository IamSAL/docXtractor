...
| Default model | `free-smart` | Most capable routing (Gemini → NIM → Groq → Mistral) |
...
export class LlmService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.defaultModel = configService.get('LLM_DEFAULT_MODEL', 'free-smart');
  }

  // Both use this.defaultModel when model param is omitted
  async extract(content, schema, systemPrompt, model?): Promise<LlmExtractionResult>
...
response = client.chat.completions.create(
    model=os.getenv("LLM_DEFAULT_MODEL", "free-smart"),
