...
import { ExecutionContext } from '../nodes/interfaces/node.interface';
...
export class WorkflowExecutorService {
   ...
   */
  private buildExecutionGraph(definition: any): Map<string, GraphNode> {
  ...
  private async executeNode(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          const result = await nodeImpl.execute(
            node.params,
            inputData,
            context,
          );
   ...
   */
  private gatherInputData(node: GraphNode, context: ExecutionContext): any {
